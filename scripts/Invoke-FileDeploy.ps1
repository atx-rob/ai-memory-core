...
function Invoke-FileDeploy {
    ...
    $ai_backup_dir = Join-Path $ai_root '.ai_memory/Temp/backup'
    ...
    foreach ($ai_rel in $Files.Keys) {
        ...
        if (Test-Path $ai_abs) {
            ...
            if (-not (Test-Path (Split-Path $ai_dest -Parent))) { 
                New-Item -ItemType Directory -Path (Split-Path $ai_dest -Parent) -Force | Out-Null 
            }
            Copy-Item $ai_abs $ai_dest -Force
        }
