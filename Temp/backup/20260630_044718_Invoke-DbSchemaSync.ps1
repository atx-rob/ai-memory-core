...
try {
  ...
  $ai_dest = Join-Path $ai_root ".ai_memory/testing/outputs/schema/${ai_target_table}_schema.md"
      # Ensure output directory exists
    $ai_output_dir = Join-Path $ai_root ".ai_memory/testing/outputs/schema"
    New-Item -ItemType Directory -Path $ai_output_dir -Force | Out-Null
     ($ai_md -join "`n"), [System.Text.UTF8Encoding]::new($false))
