...
## SECTION 1: POWERSHELL EXECUTION PROTOCOL (CONTINUED)

### Rule 1.13: Regex Pattern Pre-Flight Verification (CRITICAL)
BEFORE attempting any -replace operation on a file:
1. Use Select-String to verify the pattern exists in the target file
2. If pattern not found, DO NOT proceed with replacement
3. Report the exact pattern that was searched for
4. Either: (a) abort and request manual intervention, or (b) use a broader pattern match

**Example:**
$ai_pattern = 'const formatPhoneNumber'
if (Select-String -Path $ai_file -Pattern $ai_pattern -Quiet) {
    # Pattern found, safe to replace
    $ai_content = $ai_content -replace $ai_pattern, $ai_replacement
} else {
    Write-Host "ERROR: Pattern not found: $ai_pattern" -ForegroundColor Red
    Write-Host "File may have been manually edited or previous injection failed" -ForegroundColor Yellow
    return
}

**Why This Rule Exists:**
- PowerShell mangling (backticks, ${...}) can corrupt injected code
- Blind -replace operations on non-matching patterns cause silent failures
- This rule ensures empirical verification before modification
- Prevents "WARNING: Could not find function" errors
## SECTION 19: THE BULLETPROOF WORKFLOW (CRITICAL)

### 19.1 Pre-Edit State Verification (The "Hash & Schema" Check)
AI context degrades over long sessions. NEVER assume the current state of a file or database.
1. File Layer: Before modifying an existing file, read its current content (or verify its SHA256 hash) to ensure it matches expectations.
2. Data Layer: Before writing React code that touches the database, run `amf syncdb <table>`.
3. Type Layer: Read the resulting schema file. Identify ALL columns with non-text types (date, integer, boolean, numeric).

### 19.2 Supabase Type-Mapping Matrix
When mapping React form state to Supabase columns, the AI MUST apply these conversions:
- **Text:** Send as-is (or `null` if empty and required).
- **Date/Timestamp:** Convert `""` (empty string) to `null`. Supabase rejects `""` for date columns.
- **Integer/Numeric:** Convert `""` to `null`. If a value exists, use `parseInt()` or `parseFloat()`.
- **Boolean:** Ensure it is a strict `true`/`false`, not a string `"true"`.

### 19.3 Safe Code Generation (No Mangling)
- NEVER use double-quoted Here-Strings (`@"..."@`) or double-quoted arrays for JavaScript/JSX.
- ALWAYS use single-quoted arrays (`@('line1', 'line2')`) joined by newlines, or single-quoted Here-Strings (`@'...'@`).
- If a file is under 150 lines, rewrite the whole file using a single-quoted array. Do not use complex Regex to patch it.

### 19.4 The "Holy Trinity" Post-Write Verification
After writing code, the AI MUST run these three commands in order. If any fail, the AI must fix the code immediately.
1. `npm run build` (Verifies syntax).
2. `amf test` (Verifies logic and DB interactions).
3. `amf audit` (Verifies code matches DB schema).
## SECTION 21: ZERO-TRUST FILE WRITE PROTOCOL (CRITICAL)
To prevent PowerShell mangling and silent corruption, the AI MUST use the "Predict -> Write -> Verify" loop for all critical file writes.
1. Predict: Calculate SHA256 hash of the content variable in memory BEFORE writing.
2. Write: Write to disk using [System.IO.File]::WriteAllText.
3. Verify: Read the file back from disk and calculate its SHA256 hash.
4. Compare: If Expected Hash != Actual Hash, ABORT immediately and output a Red error: "PowerShell mangled the file! Hash mismatch!"

Example Implementation:
    $ai_bytes = [System.Text.UTF8Encoding]::new($false).GetBytes($ai_content)
    $ai_expected = [BitConverter]::ToString([System.Security.Cryptography.SHA256]::Create().ComputeHash($ai_bytes)).Replace("-", "").ToLower()
    [System.IO.File]::WriteAllText($ai_path, $ai_content, [System.Text.UTF8Encoding]::new($false))
    $ai_actual = (Get-FileHash $ai_path -Algorithm SHA256).Hash.ToLower()
    if ($ai_expected -ne $ai_actual) { Write-Host "HASH MISMATCH - ABORTING" -ForegroundColor Red; return }