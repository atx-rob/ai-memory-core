...
export default function GunForm({ userId, initialData, onEditComplete }) {
  const { addGun, updateGun, uploadImage } = useInventoryStore();
  ...
  const [formData, setFormData] = useState({
    ...
    resale_contact_phone: '', highlights: '', image_urls: []
  });

  useEffect(() => {
    if (initialData) {
      setFormData({
        name: initialData.name || '', caliber: initialData.caliber || '',
        type: initialData.type || 'Pistol', serial_number: initialData.serial_number || '',
        mfg_name: initialData.mfg_name || '', year_produced: initialData.year_produced || '',
        reload_eligible: initialData.reload_eligible || false,
        date_purchased: initialData.date_purchased || '', year_purchased: initialData.year_purchased || '',
        purchased_from: initialData.purchased_from || '', purchase_method: initialData.purchase_method || '',
        appraised_value: initialData.appraised_value || '', appraisal_year: initialData.appraisal_year || '',
  ...
  const handleFileChange = (e) => {
    ...
    // Limit to 2 images total (existing + new)
    const totalImages = (formData.image_urls || []).length + files.length;
    if (totalImages > 2) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (newFiles.length > 0) {
        const gunId = initialData ? initialData.id : Date.now(); // Use temp ID for new guns
        for (const file of newFiles) {
          const url = await uploadImage(file, userId, gunId);
  ...
  return (
    <div style={{maxWidth: '600px', margin: '0 auto 40px', padding: '30px', backgroundColor: 'var(--bg-secondary)', borderRadius: 'var(--border-radius)', border: '1px solid var(--border-color)'}}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{marginBottom: '20px', padding: '15px', backgroundColor: 'var(--bg-color)', borderRadius: 'var(--border-radius)'}}>
          ...
          {formData.image_urls && formData.image_urls.length > 0 && (
            <div style={{display: 'flex', gap: '10px', marginBottom: '10px'}}>
              {formData.image_urls.map((url, idx) => (
                <img key={idx} src={url} alt={`Gun ${idx + 1}`} style={{width: '100px', height: '100px', objectFit: 'cover', borderRadius: 'var(--border-radius)'}} />
              ))}
            </div>
          )}
          <input 
            ...
            disabled={isUploading || (formData.image_urls || []).length >= 2}
            style={{width: '100%'}}
          />
          {newFiles.length > 0 && <p style={{marginTop: '5px', fontSize: '0.9em', color: 'var(--accent-color)'}}>{newFiles.length} new file(s) selected</p>}
        </div>
        ...
        <button type="submit" disabled={isUploading} style={{width: '100%', padding: '15px', backgroundColor: 'var(--accent-color)', color: 'white', border: 'none', borderRadius: 'var(--border-radius)', fontSize: 'var(--font-size-lg)', fontWeight: '600', cursor: 'pointer', opacity: isUploading ? 0.7 : 1}}>
          {isUploading ? 'Uploading...' : (isEditMode ? 'Update Gun' : 'Add to Inventory')}
        </button>
      </form>
    </div>
  );
