...
## Rule 1.12: Script Modification Protocol
**CRITICAL:** When modifying existing PowerShell scripts:
1. **NEVER** use -replace on complex code blocks or file paths containing spaces/brackets.
2. **ALWAYS** use [regex]::Escape() if literal string replacement is required.
3. **PREFERRED:** If a script is short (< 50 lines), rewrite the entire file using a Here-String to guarantee syntax validity.
4. **NEVER** inject code inside function call parentheses.